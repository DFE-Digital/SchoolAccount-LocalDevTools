...
To run the database place a suitable `.BAK` file into the root of this project and run the command:

`docker compose up -d`

The database server will automatically restart on reboot, but can be manually stopped via 

`docker compose stop` or removed and completely and reset via `docker compose down -v` 
